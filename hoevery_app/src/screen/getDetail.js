...
const getDetail = ({ route, navigation }) => {
    ...
    const [carLatitude, setCarLatitude] = useState();
    const [carLongitude, setCarLongitude] = useState();
    ...
    const getDetailCar = async () => {
        ...
        const response = await fetch(
            `http://203.150.107.212/tenant/get-detail?car_id=${route.params.car_id}`,
            requestOptions,
        );
        const result = await response.json();
        try {
            if (result.ret == 0) {
                setCarLatitude(result.data.latitude)
                setCarLongitude(result.data.loading)
                ...
                <View styles={styles1.body_shadow}>
                    {/* <MapView
                        ...
                        <MapViewDirections
                            ...
                            onStart={params => {
                                console.log(
                                    `Started routing between "${latitude}, ${longitude} " and "${carLatitude}, ${carLongitude}"`,
                                );
                            }}
                            onReady={result => {
