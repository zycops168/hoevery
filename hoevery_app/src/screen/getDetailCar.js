...
export default class getDetailCar extends Component {
  ...
  getUserData = async () => {
    ...
    this.setState({loading: true});
    const response = await fetch(
      `http://203.150.107.212/user/info/${this.props.route.params.username}`,
      requestOptions,
    );
    const result = await response.json();
    try {
  ...
  render() {
    const {username, car_id} = this.props.route.params;
    return (
            ...
            <Overlay
              ...
              <View style={styles.overlay_container}>
                <TouchableOpacity
                  ...
                  <Text style={styles.text_loading}>กำลังดำเนินการ</Text>
                </TouchableOpacity>
              </View>
            </Overlay>
          </TouchableOpacity>
